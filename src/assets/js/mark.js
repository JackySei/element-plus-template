  ...
  for (let i = 0; i < e.length; i++) {
    ...
    if (
      ...
      ((_a = element.parentElement) === null || _a === void 0
        ? void 0
        : _a.nodeName) !== 'SCRIPT' &&
      ((_b = element.textContent) === null || _b === void 0
        ? void 0
        : _b.trim()) !== ''
    ) {
...
OutputLine.prototype.clone_empty = function () {
  const line = new OutputLine(this.__parent);
...
OutputLine.prototype.has_match = function (pattern) {
  for (
    let lastCheckedOutput = this.__items.length - 1;
    lastCheckedOutput >= 0;
    lastCheckedOutput--
  ) {
...
OutputLine.prototype._allow_wrap = function () {
  if (this._should_wrap()) {
    ...
    const next = this.__parent.current_line;
    next.set_indent(
...
OutputLine.prototype.push = function (item) {
  ...
  const last_newline_index = item.lastIndexOf('\n');
...
IndentStringCache.prototype.get_indent_size = function (indent, column) {
  let result = this.__base_string_length;
  column = column || 0;
  if (indent < 0) {
    result = 0;
  }
...
IndentStringCache.prototype.__add_column = function () {
  let column = this.__cache.length;
  let indent = 0;
  let result = '';
...
Output$3.prototype.get_code = function (eol) {
  ...
  let sweet_code = this.__lines.join('\n');
...
Output$3.prototype.remove_indent = function (index) {
  const output_length = this.__lines.length;
...
Output$3.prototype.ensure_empty_line_above = function (starts_with, ends_with) {
  let index = this.__lines.length - 2;
  while (index >= 0) {
    const potentialEmptyLine = this.__lines[index];
...
const Token_1 = Token$2;

const token = {
  Token: Token_1,
};

const acorn = createCommonjsModule(function (module, exports) {
  ...
  const baseASCIIidentifierStartChars =
    '\\x23\\x24\\x40\\x41-\\x5a\\x5f\\x61-\\x7a';

  // inside an identifier @ is not allowed but 0-9 are.
  const baseASCIIidentifierChars =
...
Options$9.prototype._get_boolean = function (name, default_value) {
  const option_value = this.raw_options[name];
  const result = option_value === undefined ? !!default_value : !!option_value;
  return result;
};
...
Options$9.prototype._get_number = function (name, default_value) {
  ...
  let result = parseInt(option_value, 10);
  if (isNaN(result)) {
...
Options$9.prototype._get_selection = function (
  name,
  selection_list,
  default_value
) {
...
Options$9.prototype._get_selection_list = function (
  name,
  selection_list,
  default_value
) {
...
const Options_1$3 = Options$9;
const normalizeOpts = _normalizeOpts;
const mergeOpts = _mergeOpts;
...
const BaseOptions$2 = options$3.Options;

const validPositionValues$1 = [
  'before-newline',
  'after-newline',
  'preserve-newline',
];
...
InputScanner$3.prototype.match = function (pattern) {
  let pattern_match = this.__match(pattern, this.__position);
  if (pattern_match) {
...
InputScanner$3.prototype.read = function (
  starting_pattern,
  until_pattern,
  until_after
) {
...
InputScanner$3.prototype.readUntil = function (pattern, until_after) {
  let val = '';
  let match_index = this.__position;
  pattern.lastIndex = this.__position;
  const pattern_match = pattern.exec(this.__input);
  if (pattern_match) {
...
const InputScanner_1 = InputScanner$3;
...
const TokenStream_1 = TokenStream$1;
...
Pattern$4.prototype.read = function () {
  let result = this._input.read(this._starting_pattern);
...
const Pattern_1 = Pattern$4;

const pattern = {
  Pattern: Pattern_1,
};

const Pattern$3 = pattern.Pattern;
...
WhitespacePattern$1.prototype.__set_whitespace_patterns = function (
  whitespace_chars,
  newline_chars
) {
  whitespace_chars += '\\t ';
...
WhitespacePattern$1.prototype.read = function () {
  ...
  const resulting_string = this._input.read(this._match_pattern);
  if (resulting_string === ' ') {
    this.whitespace_before_token = ' ';
  } else if (resulting_string) {
    const matches = this.__split(this._newline_regexp, resulting_string);
...
WhitespacePattern$1.prototype.matching = function (
  whitespace_chars,
  newline_chars
) {
  const result = this._create();
...
WhitespacePattern$1.prototype.__split = function (regexp, input_string) {
  ...
  let start_index = 0;
  const result = [];
  let next_match = regexp.exec(input_string);
  while (next_match) {
...
const WhitespacePattern_1 = WhitespacePattern$1;

const whitespacepattern = {
  WhitespacePattern: WhitespacePattern_1,
};

const InputScanner$2 = inputscanner.InputScanner;
const Token$1 = token.Token;
const TokenStream = tokenstream.TokenStream;
const WhitespacePattern = whitespacepattern.WhitespacePattern;
...
const Tokenizer$4 = function (input_string, options) {
...
Tokenizer$4.prototype.tokenize = function () {
  ...
  let current;
  let previous = new Token$1(TOKEN$4.START, '');
  let open_token = null;
  const open_stack = [];
...
const Tokenizer_1$2 = Tokenizer$4;
const TOKEN_1$2 = TOKEN$4;
...
function Directives$3(start_block_pattern, end_block_pattern) {
  ...
  this.__directives_end_ignore_pattern = new RegExp(
    start_block_pattern + /\sbeautify\signore:end\s/.source + end_block_pattern,
    'g'
  );
}
...
Directives$3.prototype.get_directives = function (text) {
  ...
  let directive_match = this.__directive_pattern.exec(text);
...
const Directives_1 = Directives$3;

const directives = {
  Directives: Directives_1,
};

const Pattern$2 = pattern.Pattern;
...
function TemplatablePattern$2(input_scanner, parent) {
  ...
  const pattern = new Pattern$2(input_scanner);
  this.__patterns = {
...
TemplatablePattern$2.prototype.read_options = function (options) {
  const result = this._create();
  for (const language in template_names) {
...
const TemplatablePattern_1 = TemplatablePattern$2;

const templatablepattern = {
  TemplatablePattern: TemplatablePattern_1,
};

const InputScanner$1 = inputscanner.InputScanner;
const BaseTokenizer$1 = tokenizer$2.Tokenizer;
const BASETOKEN$1 = tokenizer$2.TOKEN;
const Directives$2 = directives.Directives;

const Pattern$1 = pattern.Pattern;
const TemplatablePattern$1 = templatablepattern.TemplatablePattern;
...
const directives_core$2 = new Directives$2(/\/\*/, /\*\//);
...
const positionable_operators$1 = (
  '>>> === !== ' +
  '<< && >= ** != == <= >> || ?? |> ' +
  '< / - + > : & % ? ^ | *'
).split(' ');
...
const Tokenizer$3 = function (input_string, options) {
  ...
  const pattern_reader = new Pattern$1(this._input);
  const templatable = new TemplatablePattern$1(this._input).read_options(
    this._options
  );
...
Tokenizer$3.prototype._is_comment = function (current_token) {
  return (
    current_token.type === TOKEN$3.COMMENT ||
    current_token.type === TOKEN$3.BLOCK_COMMENT ||
    current_token.type === TOKEN$3.UNKNOWN
  );
};

Tokenizer$3.prototype._is_opening = function (current_token) {
  return (
    current_token.type === TOKEN$3.START_BLOCK ||
    current_token.type === TOKEN$3.START_EXPR
  );
};

Tokenizer$3.prototype._is_closing = function (current_token, open_token) {
  return (
    (current_token.type === TOKEN$3.END_BLOCK ||
      current_token.type === TOKEN$3.END_EXPR) &&
    open_token &&
...
Tokenizer$3.prototype._read_word = function (previous_token) {
  ...
  if (resulting_string !== '') {
    ...
    if (
      !(
        previous_token.type === TOKEN$3.DOT ||
        (previous_token.type === TOKEN$3.RESERVED &&
...
Tokenizer$3.prototype._read_punctuation = function () {
  const resulting_string = this.__patterns.punct.read();
...
Tokenizer$3.prototype._read_comment = function (c) {
  ...
  if (c === '/') {
    ...
    if (this._input.peek(1) === '*') {
      ...
      const directives = directives_core$2.get_directives(comment);
...
Tokenizer$3.prototype._read_string = function (c) {
  if (c === '`' || c === "'" || c === '"') {
    let resulting_string = this._input.next();
...
Tokenizer$3.prototype._allow_regexp_or_xml = function (previous_token) {
  ...
  return (
    (previous_token.type === TOKEN$3.RESERVED &&
      in_array$2(previous_token.text, [
        'return',
        'case',
        'throw',
        'else',
    ...
    (previous_token.type === TOKEN$3.END_EXPR &&
      previous_token.text === ')' &&
      previous_token.opened.previous.type === TOKEN$3.RESERVED &&
      in_array$2(previous_token.opened.previous.text, [
        'if',
        'while',
        'for',
      ])) ||
    in_array$2(previous_token.type, [
      TOKEN$3.COMMENT,
      TOKEN$3.START_EXPR,
      TOKEN$3.START_BLOCK,
      TOKEN$3.START,
...
Tokenizer$3.prototype._read_xml = function (c, previous_token) {
  if (
    ...
    let xmlStr = '';
    let match = this.__patterns.xml.read_match();
    // handle e4x xml literals
    //
    if (match) {
      // Trim root tag to attempt to
      const rootTag = match[2].replace(/^{\s+/, '{').replace(/\s+}$/, '}');
      const isCurlyRoot = rootTag.indexOf('{') === 0;
      let depth = 0;
      while (match) {
        const isEndTag = !!match[1];
        const tagName = match[2];
        const isSingletonTag =
...
function unescape_string(s) {
  ...
  const input_scan = new InputScanner$1(s);
  let matched = null;
...
// handle string
//
Tokenizer$3.prototype._read_string_recursive = function (
  delimiter,
  ...
  let resulting_string = pattern.read();
  let next = '';
...
const Tokenizer_1$1 = Tokenizer$3;
const TOKEN_1$1 = TOKEN$3;
const positionable_operators_1 = positionable_operators$1.slice();
const line_starters_1 = line_starters$1.slice();
...
const Output$2 = output.Output;
const Token = token.Token;

const Options$7 = options$2.Options;
const Tokenizer$2 = tokenizer$1.Tokenizer;
const line_starters = tokenizer$1.line_starters;
const positionable_operators = tokenizer$1.positionable_operators;
const TOKEN$2 = tokenizer$1.TOKEN;
...
function reserved_array(token, words) {
  return (
    token && token.type === TOKEN$2.RESERVED && in_array$1(token.text, words)
  );
}
...
const OPERATOR_POSITION_BEFORE_OR_PRESERVE = [
...
Beautifier$5.prototype.create_flags = function (flags_base, mode) {
  ...
  const next_flags = {
    ...
    last_token: flags_base
      ...
      : new Token(TOKEN$2.START_BLOCK, ''), // last token text
...
Beautifier$5.prototype._reset = function (source_text) {
  ...
  const tokenizer = new Tokenizer$2(source_text, this._options);
...
Beautifier$5.prototype.handle_token = function (
  current_token,
...
Beautifier$5.prototype.handle_whitespace_and_comments = function (
  current_token,
  preserve_statement_flags
) {
...
Beautifier$5.prototype.allow_wrap_or_preserved_newline = function (
  current_token,
  force_linewrap
) {
...
Beautifier$5.prototype.print_newline = function (
  force_newline,
  preserve_statement_flags
) {
...
Beautifier$5.prototype.print_token = function (current_token) {
  ...
  if (
    ...
    current_token.previous.type === TOKEN$2.COMMA &&
    this._output.just_added_newline()
  ) {
...
Beautifier$5.prototype.start_of_statement = function (current_token) {
  ...
  start =
    start ||
    (this._flags.last_token.type === TOKEN$2.WORD &&
      this._flags.mode === MODE.BlockStatement &&
      !this._flags.in_case &&
      !(current_token.text === '--' || current_token.text === '++') &&
      this._last_last_text !== 'function' &&
      current_token.type !== TOKEN$2.WORD &&
      current_token.type !== TOKEN$2.RESERVED);
  start =
    start ||
...
Beautifier$5.prototype.handle_start_expr = function (current_token) {
  ...
  if (current_token.text === '[') {
    ...
    if (
      !in_array$1(this._flags.last_token.type, [
        TOKEN$2.START_EXPR,
        TOKEN$2.END_EXPR,
        TOKEN$2.WORD,
        TOKEN$2.OPERATOR,
    ...
    if (this._flags.last_token.type === TOKEN$2.RESERVED) {
      if (this._flags.last_token.text === 'for') {
        ...
        in_array$1(this._flags.last_token.text, ['if', 'while', 'switch'])
      ) {
        this._output.space_before_token =
          this._options.space_before_conditional;
  ...
  if (
    this._flags.last_token.text === ';' ||
    this._flags.last_token.type === TOKEN$2.START_BLOCK
  ) {
    this.print_newline();
  } else if (
    this._flags.last_token.type === TOKEN$2.END_EXPR ||
    this._flags.last_token.type === TOKEN$2.START_EXPR ||
    this._flags.last_token.type === TOKEN$2.END_BLOCK ||
    this._flags.last_token.text === '.' ||
    this._flags.last_token.type === TOKEN$2.COMMA
  ) {
...
Beautifier$5.prototype.handle_start_block = function (current_token) {
  ...
  if (
    ...
    second_token &&
    ((in_array$1(second_token.text, [':', ',']) &&
      in_array$1(next_token.type, [
        TOKEN$2.STRING,
        TOKEN$2.WORD,
        TOKEN$2.RESERVED,
      ])) ||
      (in_array$1(next_token.text, ['get', 'set', '...']) &&
        in_array$1(second_token.type, [TOKEN$2.WORD, TOKEN$2.RESERVED])))
  ) {
  ...
  const empty_anonymous_function =
    empty_braces &&
    this._flags.last_word === 'function' &&
    this._flags.last_token.type === TOKEN$2.END_EXPR;
  ...
  if (this._options.brace_preserve_inline) {
    ...
    do {
      ...
      check_token.type !== TOKEN$2.EOF &&
      !(
        check_token.type === TOKEN$2.END_BLOCK &&
...
Beautifier$5.prototype.handle_end_block = function (current_token) {
  ...
  const empty_braces = this._flags.last_token.type === TOKEN$2.START_BLOCK;
...
Beautifier$5.prototype.handle_word = function (current_token) {
  if (current_token.type === TOKEN$2.RESERVED) {
    if (
      in_array$1(current_token.text, ['set', 'get']) &&
      this._flags.mode !== MODE.ObjectLiteral
    ) {
      current_token.type = TOKEN$2.WORD;
    } else if (
      ...
      in_array$1(current_token.text, ['as', 'from']) &&
  ...
  if (reserved_word(current_token, 'function')) {
    if (
      in_array$1(this._flags.last_token.text, ['}', ';']) ||
      (this._output.just_added_newline() &&
        !(
          in_array$1(this._flags.last_token.text, [
            '(',
  ...
  if (this._flags.last_token.type === TOKEN$2.END_BLOCK) {
    ...
    this._flags.last_token.type === TOKEN$2.SEMICOLON &&
    this._flags.mode === MODE.BlockStatement
  ) {
  ...
  if (
    current_token.previous &&
    (current_token.previous.type === TOKEN$2.WORD ||
      current_token.previous.type === TOKEN$2.RESERVED)
  ) {
    this._output.space_before_token = true;
  }
  this.print_token(current_token);
...
Beautifier$5.prototype.handle_operator = function (current_token) {
  ...
  if (
    current_token.text === '*' &&
    this._flags.last_token.type === TOKEN$2.DOT
  ) {
    this.print_token(current_token);
    return;
  }
  ...
  if (isGeneratorAsterisk) {
    ...
    space_after =
      next_token &&
      in_array$1(next_token.type, [TOKEN$2.WORD, TOKEN$2.RESERVED]);
  } else if (current_token.text === '...') {
    this.allow_wrap_or_preserved_newline(current_token);
...
Beautifier$5.prototype.handle_block_comment = function (
  current_token,
  preserve_statement_flags
) {
  if (this._output.raw) {
...
Beautifier$5.prototype.print_block_commment = function (
  current_token,
  preserve_statement_flags
) {
...
Beautifier$5.prototype.handle_comment = function (
  current_token,
  preserve_statement_flags
) {
...
Beautifier$5.prototype.handle_unknown = function (
  current_token,
  preserve_statement_flags
) {
  this.print_token(current_token);
...
const Beautifier_1$2 = Beautifier$5;

const beautifier$2 = {
  Beautifier: Beautifier_1$2,
};

const Beautifier$4 = beautifier$2.Beautifier;
const Options$6 = options$2.Options;

function js_beautify(js_source_text, options) {
  const beautifier = new Beautifier$4(js_source_text, options);
  return beautifier.beautify();
}

const javascript = js_beautify;
const defaultOptions$2 = function () {
  return new Options$6();
};
javascript.defaultOptions = defaultOptions$2;

const BaseOptions$1 = options$3.Options;
...
function Options$5(options) {
  ...
  const space_around_selector_separator = this._get_boolean(
...
const Options_1$1 = Options$5;

const options$1 = {
  Options: Options_1$1,
};

const Options$4 = options$1.Options;
const Output$1 = output.Output;
const InputScanner = inputscanner.InputScanner;
const Directives$1 = directives.Directives;

const directives_core$1 = new Directives$1(/\/\*/, /\*\//);

const lineBreak$1 = /\r\n|[\r\n]/;
const allLineBreaks$1 = /\r\n|[\r\n]/g;
...
Beautifier$3.prototype.eatWhitespace = function (allowAtLeastOneNewLine) {
  const result = whitespaceChar.test(this._input.peek());
  let newline_count = 0;
...
Beautifier$3.prototype.beautify = function () {
  ...
  let source_text = this._source_text;
  let eol = this._options.eol;
  if (eol === 'auto') {
    eol = '\n';
  ...
  let insideAtExtend = false;
  let insideAtImport = false;
  let topCharacter = this._ch;
  let whitespace;
  let isAfterSpace;
  let previous_ch;
  ...
  while (true) {
    ...
    if (!this._ch) {
      ...
      let comment = this._input.read(block_comment_pattern);

      // Handle ignore directive
      const directives = directives_core$1.get_directives(comment);
      if (directives && directives.ignore === 'start') {
      ...
      if (this._input.peek() === '{') {
        ...
        let variableOrRule = this._input.peekUntilAfter(/[: ,;{}()[\]\/='"]/g);
  ...
  const sweetCode = this._output.get_code(eol);

  return sweetCode;
};

const Beautifier_1$1 = Beautifier$3;

const beautifier$1 = {
  Beautifier: Beautifier_1$1,
};

const Beautifier$2 = beautifier$1.Beautifier;
const Options$3 = options$1.Options;

function css_beautify(source_text, options) {
  const beautifier = new Beautifier$2(source_text, options);
  return beautifier.beautify();
}

const css$1 = css_beautify;
const defaultOptions$1 = function () {
  return new Options$3();
};
css$1.defaultOptions = defaultOptions$1;

const BaseOptions = options$3.Options;
...
const Options_1 = Options$2;

const options = {
  Options: Options_1,
};

const BaseTokenizer = tokenizer$2.Tokenizer;
const BASETOKEN = tokenizer$2.TOKEN;
const Directives = directives.Directives;
const TemplatablePattern = templatablepattern.TemplatablePattern;
...
const Tokenizer$1 = function (input_string, options) {
  BaseTokenizer.call(this, input_string, options);
  this._current_tag_name = '';

  // Words end at whitespace or when a tag starts
  // if we are indenting handlebars, they are considered tags
  const templatable_reader = new TemplatablePattern(this._input).read_options(
    this._options
  );
  ...
  if (this._options.unformatted_content_delimiter) {
    const literal_regexp = this._input.get_literal_regexp(
...
Tokenizer$1.prototype._is_closing = function (current_token, open_token) {
  return (
    current_token.type === TOKEN$1.TAG_CLOSE &&
    open_token &&
    (((current_token.text === '>' || current_token.text === '/>') &&
      open_token.text[0] === '<') ||
      (current_token.text === '}}' &&
...
Tokenizer$1.prototype._read_attribute = function (
  c,
  previous_token,
  open_token
) {
...
Tokenizer$1.prototype._read_raw_content = function (
  c,
  previous_token,
  open_token
) {
  ...
  if (open_token && open_token.text[0] === '{') {
    ...
    previous_token.type === TOKEN$1.TAG_CLOSE &&
    previous_token.opened.text[0] === '<' &&
    previous_token.text[0] !== '/'
  ) {
    // ^^ empty tag has no content
    const tag_name = previous_token.opened.text.substr(1).toLowerCase();
    if (tag_name === 'script' || tag_name === 'style') {
      // Script and style tags are allowed to have comments wrapping their content
      // or just have regular content.
      const token = this._read_comment_or_cdata(c);
      if (token) {
...
const Options$1 = options.Options;
...
const Printer = function (options, base_indent_string) {
...
const get_type_attribute = function (start_token) {
  let result = null;
  let raw_token = start_token.next;
...
const get_custom_beautifier_name = function (tag_check, raw_token) {
...
TagStack.prototype.try_pop = function (tag, stop_list) {
  // function to retrieve the opening tag to the corresponding closer
  const frame = this._get_frame([tag], stop_list);
  return this._try_pop_frame(frame);
};

TagStack.prototype.indent_to_tag = function (tag_list) {
  const frame = this._get_frame(tag_list);
  if (frame) {
    this._printer.indent_level = frame.indent_level;
  }
};
...
function Beautifier$1(source_text, options, js_beautify, css_beautify) {
  ...
  // Allow the setting of language/file-type specific options
  // with inheritance of overall settings
  const optionHtml = new Options$1(options, 'html');
  ...
  this._is_wrap_attributes_preserve =
    this._options.wrap_attributes.substr(0, 'preserve'.length) === 'preserve';
...
Beautifier$1.prototype.beautify = function () {
  ...
  let last_tag_token = new TagOpenParserToken();

  const printer = new Printer(this._options, baseIndentString);
  const tokens = new Tokenizer(source_text, this._options).tokenize();

  this._tag_stack = new TagStack(printer);

  let parser_token = null;
  let raw_token = tokens.next();
  ...
  const sweet_code = printer._output.get_code(eol);

  return sweet_code;
};

Beautifier$1.prototype._handle_tag_close = function (
  printer,
  raw_token,
  last_tag_token
) {
...
Beautifier$1.prototype._handle_inside_tag = function (
  printer,
  raw_token,
  last_tag_token,
  tokens
) {
  let wrapped = last_tag_token.has_wrapped_attrs;
  const parser_token = {
    text: raw_token.text,
    type: raw_token.type,
  };
  ...
  if (last_tag_token.is_unformatted) {
    ...
    if (
      ...
      if (this._is_wrap_attributes_force) {
        let force_attr_wrap = last_tag_token.attr_count > 1;
        if (
          this._is_wrap_attributes_force_expand_multiline &&
          last_tag_token.attr_count === 1
        ) {
          let is_only_attribute = true;
          let peek_index = 0;
          let peek_token;
          do {
...
Beautifier$1.prototype._handle_text = function (
  printer,
  raw_token,
  last_tag_token
) {
...
Beautifier$1.prototype._print_custom_beatifier_text = function (
  printer,
  raw_token,
  last_tag_token
) {
  ...
  if (raw_token.text !== '') {
    ...
    if (
      ...
      _beautifier = function (html_source, options) {
        const beautifier = new Beautifier$1(
          html_source,
          options,
          local._js_beautify,
          local._css_beautify
    ...
    const indentation = printer.get_full_indent(script_indent_level);
    ...
    if (text) {
      if (_beautifier) {
        ...
        const Child_options = function () {
          this.eol = '\n';
        };
        Child_options.prototype = this._options.raw_options;
        const child_options = new Child_options();
        text = _beautifier(indentation + text, child_options);
      } else {
        // simply indent the string otherwise
        const white = raw_token.whitespace_before;
        if (white) {
...
Beautifier$1.prototype._handle_tag_open = function (
  printer,
  raw_token,
  last_tag_token,
  last_token
) {
  const parser_token = this._get_tag_open_token(raw_token);
...
Beautifier$1.prototype._get_tag_open_token = function (raw_token) {
  ...
  const parser_token = new TagOpenParserToken(
...
Beautifier$1.prototype._set_tag_position = function (
  printer,
  raw_token,
  parser_token,
  last_tag_token,
  ...
  if (parser_token.is_empty_element) {
    ...
    if (
      ...
      const foundIfOnCurrentLine = printer.current_line_has_match(/{{#if/);
...
Beautifier$1.prototype._calcluate_parent_multiline = function (
  printer,
  parser_token
) {
  if (
...
Beautifier$1.prototype._do_optional_end_element = function (parser_token) {
  ...
  if (parser_token.tag_name === 'body') {
    ...
    const p_parent = parser_token.parent.parent;
    ...
    result =
      result ||
      this._tag_stack.try_pop('td', ['table', 'thead', 'tbody', 'tfoot', 'tr']);
    result =
      result ||
...
const Beautifier_1 = Beautifier$1;

const beautifier = {
  Beautifier: Beautifier_1,
};

const Beautifier = beautifier.Beautifier;
...
const js$1 = javascript;
const css = css$1;
const html = style_html;
...
const js = createCommonjsModule(function (module) {
  /**
  ...
  **/
  ...
  function get_beautify(js_beautify, css_beautify, html_beautify) {
    // the default is js
    const beautify = function (src, config) {
...
function splitVTextNode(text, offset) {
  let _a;
  const i = vTextNodes.findIndex((item) => item.text === text);
  if (i !== -1) {
    ...
    const newVText = {
      ...
      offset:
        vTextNodes[i].offset +
        (((_a = vTextNodes[i].text.textContent) === null || _a === void 0
          ? void 0
          : _a.length) || 0),
    };
    ...
    // console.error(
    //   'bugï¼Œæ‰¾ä¸åˆ°å¯ä»¥splitVTextNodeçš„èŠ‚ç‚¹ï¼Œè¯·è”ç³»å¼€å‘äººå‘˜'
    // );
  }
}
function sliceVTextNode(startText, endText, hasIgnore = false) {
  const startIndex = vTextNodes.findIndex(
    (item) => item.text === startText.text
  );
  ...
  if (startIndex !== -1 && endIndex !== -1) {
    ...
    rangeTextNodes = hasIgnore
      ? rangeTextNodes.filter((item) => !item.ignore)
      : rangeTextNodes;
    return rangeTextNodes.map((item) => item.text);
  } else {
    // console.error(
    //   'bugï¼Œæ‰¾ä¸åˆ°å¯ä»¥sliceVTextNodeçš„èŠ‚ç‚¹ï¼Œè¯·è”ç³»å¼€å‘äººå‘˜'
    // );
  }
}
function getTextContainer(offset) {
  const nodes = vTextNodes.filter((item) => !item.ignore);
  ...
  if (i !== -1) {
    ...
    // console.error(
    //   'bugï¼Œæ‰¾ä¸åˆ°å¯ä»¥getTextContainerçš„èŠ‚ç‚¹ï¼Œè¯·è”ç³»å¼€å‘äººå‘˜'
    // );
  }
}
function getOptionalVText(vTextNode) {
  ...
  if (i !== -1) {
    ...
    // console.error(
    //   'bugï¼Œæ‰¾ä¸åˆ°å¯ä»¥getOptionalVTextçš„èŠ‚ç‚¹ï¼Œè¯·è”ç³»å¼€å‘äººå‘˜'
    // );
  }
}
...
class JsMark {
  constructor(ops) {
    ...
    if (ele.nodeType !== 1) {
      this._onError('è¯·æŒ‚è½½domèŠ‚ç‚¹');
    }
    if (!this._selection) {
      this._onError(
        'æµè§ˆå™¨æš‚ä¸æ”¯æŒæ ‡æ³¨ï¼Œè¯·æŸ¥çœ‹æ–‡æ¡£æ”¯æŒæµè§ˆå™¨ç‰ˆæœ¬'
      );
    }
  ...
  _onClick(e) {
    ...
    const bottomPosition = document.body.offsetHeight - topPosition;
    let rightPosition = document.body.offsetWidth - leftPosition;
    if (rightPosition < 210) {
      rightPosition = 210;
    }
    if (bottomPosition < 230) {
      topPosition = document.body.offsetHeight - 310;
    }
    if (e.target !== null && 'dataset' in e.target) {
      const selectorId = e.target.dataset.selector;
      if (selectorId) {
        const eleArr = document.querySelectorAll(
          `span[${markSelector}="${selectorId}"]`
        );
  ...
  _onMouseUp(e) {
    ...
    this._captureSelection(range, topPosition, leftPosition, rightPosition);
  }

  _captureSelection(range, topPosition, leftPosition, rightPosition) {
    const selection = this._selection;
    if (!selection) {
      return;
    }
    if (
      ...
      return this._onError('åªå¯é€‰ä¸­æ–‡æœ¬èŠ‚ç‚¹');
    }
    if (
      !config.isCover &&
      !range.storeRenderOther &&
      this.hasCoverSelector(range, markSelector)
    ) {
      selection.removeAllRanges();
      return this._onError(
        'ä¸å…è®¸è¦†ç›–æ ‡æ³¨ï¼Œè¯¦ç»†è¯·çœ‹é…ç½®æ–‡æ¡£ï¼Œæˆ–è®¾ç½®isCoverä¸ºtrue'
      );
    }
    ...
    const textNodes =
      sliceVTextNode(startText, endTextNext, config.ignoreClass.length > 0) ||
      [];
    const offset = startText.offset;
    let text = '';
    textNodes === null || textNodes === void 0
      ? void 0
    ...
    this._onSelected({
      ...
      textNodes,
      storeRenderOther:
        range && range.storeRenderOther ? range.storeRenderOther : {},
      topPosition,
      leftPosition,
      rightPosition,
    });
  ...
  hasCoverSelector(range, attrName) {
    ...
    if (range.cloneContents().querySelector(`[${attrName}]`)) {
      ...
      if (
        ((_a = range.commonAncestorContainer.parentNode) === null ||
        _a === void 0
          ? void 0
          : _a.nodeType) === 1
      ) {
  ...
  renderStore(obj) {
    ...
    obj.map((item) => {
      ...
      const startContainer = getTextContainer(item.offset);
      const endContainer = getTextContainer(end);
  ...
  repaintRange(rangeNode) {
    const { uuid, className, textNodes } = rangeNode;
    const uid = uuid || Guid();
    let offsetTop = null;
    let offsetLeft = null;
  ...
  deleteMark(uuid) {
    const eleArr = document.querySelectorAll(`span[${markSelector}="${uuid}"]`);
